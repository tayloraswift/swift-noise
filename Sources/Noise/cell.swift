fileprivate protocol _CellNoise2D {
...
extension _CellNoise2D {
    @inline(__always) private func distance2(
        from sample_point: Math.DoubleV2,
        generating_point: Math.IntV2
    ) -> Double {
        ...
        let dp: Math.DoubleV2 = (
            (Double(hash >> 4         ) - 15 / 2) * 1 / 16,
            (Double(hash      & 0b1111) - 15 / 2) * 1 / 16
        )

        let dv: Math.DoubleV2 = Math.sub(
            Math.add(Math.cast_double(generating_point), dp),
            sample_point
        )
    ...
    @inline(__always) func _closest_point(_ x: Double, _ y: Double) -> (
        point: (Int, Int),
        r2: Double
    ) {
        ...
        let nearpoint_disp: Math.DoubleV2 = (
            abs(sample_rel.x - Double((quadrant.a + 1) >> 1)),
        ...
        @inline(__always) func _inspect(
            ...
            if dx*dx + dy*dy < r2 {
                let dr2: Double = self.distance2(
                    from: sample,
                    generating_point: generating_point
                )
                if dr2 < r2 {
                    r2            = dr2
        ...
        // C points
        _inspect(
            generating_point: (inner.a, far.b),
            dx: nearpoint_disp.x + 0.5,
            dy: nearpoint_disp.y - 0.5
        )
        _inspect(
            generating_point: (far.a, inner.b),
            dx: nearpoint_disp.x - 0.5,
            dy: nearpoint_disp.y + 0.5
        )
        ...
        // E points
        _inspect(
            generating_point: (far.a, outer.b),
            dx: nearpoint_disp.x - 0.5,
            dy: nearpoint_disp.y - 1.5
        )
        _inspect(
            generating_point: (outer.a, far.b),
            dx: nearpoint_disp.x - 1.5,
            dy: nearpoint_disp.y - 0.5
        )
...
public struct CellNoise2D: _CellNoise2D, HashedNoise {
    ...
    public func closest_point(_ x: Double, _ y: Double) -> (point: (Int, Int), r2: Double) {
...
public struct TilingCellNoise2D: _CellNoise2D, HashedTilingNoise {
    let permutation_table: PermutationTable,
    amplitude: Double,
    frequency: Double,
...
fileprivate protocol _CellNoise3D {
    var frequency: Double { get }
    var amplitude: Double { get }
...
extension _CellNoise3D {
    @inline(__always) private func distance2(
        from sample_point: Math.DoubleV3,
        generating_point: Math.IntV3
    ) -> Double {
        let hash: Int = self.hash(point: generating_point)
        // hash is within 0 ... 255, take it to 0 ... 0.5
        ...
        let axes: Math.DoubleV3 = Math.cast_double(
            (
                hash >> 5,
                hash >> 2 & 0b0111,
        ...
        let dp: Math.DoubleV3 = (
            (axes.x - 7 / 2) * 1.0 / 8,
            (axes.y - 7 / 2) * 1.0 / 8,
            (axes.z - 7 / 2) * 1.0 / 8
        )

        let dv: Math.DoubleV3 = Math.sub(
            Math.add(Math.cast_double(generating_point), dp),
            sample_point
        )
        return Math.dot(dv, dv)
    ...
    @inline(__always) func _closest_point(_ x: Double, _ y: Double, _ z: Double) -> (
        point: (Int, Int, Int),
        r2: Double
    ) {
        ...
        let quadrant: Math.IntV3  = (
            sample_rel.x > 0.5 ? 1 : -1,
            sample_rel.y > 0.5 ? 1 : -1,
            sample_rel.z > 0.5 ? 1 : -1
        )
        let near: Math.IntV3      = Math.add(
            bin, (
                (quadrant.a + 1) >> 1,
                (quadrant.b + 1) >> 1,
                (quadrant.c + 1) >> 1
        ...
        let nearpoint_disp: Math.DoubleV3 = (
            abs(sample_rel.x - Double((quadrant.a + 1) >> 1)),
            abs(sample_rel.y - Double((quadrant.b + 1) >> 1)),
        ...
        @inline(__always) func _inspect_cell(offset: Math.IntV3) {
            // calculate distance from quadrant volume to kernel cell
            var cell_distance2: Double
            if offset.a != 0 {                                                                // move by 0.5 towards zero
                let dx: Double = nearpoint_disp.x + Double(offset.a) + (
                    offset.a > 0 ? -0.5 : 0.5
                )
            ...
            if offset.b != 0 {                                                                // move by 0.5 towards zero
                let dy: Double = nearpoint_disp.y + Double(offset.b) + (
                    offset.b > 0 ? -0.5 : 0.5
                )
                cell_distance2 += dy*dy
            }

            if offset.c != 0 {                                                                // move by 0.5 towards zero
                let dz: Double = nearpoint_disp.z + Double(offset.c) + (
                    offset.c > 0 ? -0.5 : 0.5
                )
                cell_distance2 += dz*dz
            }

            guard cell_distance2 < r2 else {
                return
            }
...
/// units.
public struct CellNoise3D: _CellNoise3D, HashedNoise {
    let permutation_table: PermutationTable,
    amplitude: Double,
    frequency: Double
    ...
    /// be fed to a color hashing function to produce a
    /// [Voronoi diagram](https://en.wikipedia.org/wiki/Voronoi_diagram).
    public func closest_point(_ x: Double, _ y: Double, _ z: Double) -> (
        point: (Int, Int, Int),
        r2: Double
    ) {
...
public struct TilingCellNoise3D: _CellNoise3D, HashedTilingNoise {
    let permutation_table: PermutationTable,
    amplitude: Double,
    frequency: Double,
