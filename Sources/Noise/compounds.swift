...
public struct FBM<Source>: Noise where Source: Noise {
    private let generators: [Source]

    // UNDOCUMENTED
    public func amplitude_scaled(by factor: Double) -> FBM {
        return FBM<Source>(generators: self.generators.map{ $0.amplitude_scaled(by: factor) })
    }
    public func frequency_scaled(by factor: Double) -> FBM {
        return FBM<Source>(generators: self.generators.map{ $0.frequency_scaled(by: factor) })
    }
    public func reseeded() -> FBM {
        return FBM<Source>(generators: self.generators.map{ $0.reseeded() })
    }

    private init(generators: [Source]) {
    ...
    public init(
        ...
        for i in (0 ..< octaves - 1) {
            generators.append(
                generators[i].amplitude_scaled(by: persistence).frequency_scaled(
                    by: lacunarity
                ).reseeded()
            )
        }
...
extension FBM where Source: TilingNoise {
    public init(tiling source: Source, octaves: Int, persistence: Double = 0.5) {
        // calculate maximum range
        let range_inverse: Double
        if persistence == 0.5 {
            range_inverse = Double(1 << (octaves - 1)) / Double(1 << octaves - 1)
        ...
        for i in (0 ..< octaves - 1) {
            generators.append(
                generators[i].amplitude_scaled(by: persistence)
...
// UNDOCUMENTED
public struct DistortedNoise<Source, Displacement>: Noise where Source: Noise,
    Displacement: Noise {
    private let source: Source,
    displacement: Displacement

    public func amplitude_scaled(by factor: Double) -> DistortedNoise<Source, Displacement> {
        return DistortedNoise<Source, Displacement>(
            displacing: self.source.amplitude_scaled(by: factor),
            with: self.displacement
        )
    }
    public func frequency_scaled(by factor: Double) -> DistortedNoise<Source, Displacement> {
        return DistortedNoise<Source, Displacement>(
            displacing: self.source.frequency_scaled(by: factor),
            with: self.displacement.frequency_scaled(by: factor)
                .amplitude_scaled(by: factor)
        )
    }
    public func reseeded() -> DistortedNoise<Source, Displacement> {
        return DistortedNoise<Source, Displacement>(
            displacing: self.source.reseeded(),
            with: self.displacement.reseeded()
        )
    }

    public init(displacing source: Source, with displacement: Displacement) {
        self.source       = source
...
extension DistortedNoise where Source == Displacement {
    public init(_ source: Source, strength: Double) {
        self.source       = source
