...
public struct DiskSampler2D {
    private let candidate_ring: [Math.DoubleV2]

    private var rng: RandomXorshift,
    candidate_index: Int = 0

    private var candidate_offset: Math.DoubleV2 {
        return self.candidate_ring[self.candidate_index]
    }

    private static let candidate_table_bitmask: Int = 0b1111111111 // 1023
    ...
    public mutating func generate(
        ...
        _ = DiskSampler2D.attempt_insert(
            candidate: queue[0],
            into_grid: &grid,
        ...
        outer: while let front: Math.DoubleV2 = queue.last {
            for _ in 0 ..< k {
                ...
                ) & DiskSampler2D.candidate_table_bitmask

                guard 0 ..< normalized_width ~= candidate.x && 0 ..< normalized_height ~= candidate.y else {
                    continue
                }

                if DiskSampler2D.attempt_insert(
                        candidate: candidate,
                        into_grid: &grid,
                        grid_stride: grid_stride
                    ) {
                    ...
                    queue.swapAt(
                        queue.endIndex - 1,
                        Int(self.rng.generate(less_than: UInt32(queue.endIndex)))
                    )
                    continue outer
                }
            }
    ...
    private static func attempt_insert(
        candidate: Math.DoubleV2,
        into_grid grid: inout [Math.DoubleV2?],
        grid_stride: Int
    ) -> Bool {
        ...
        let ring: [Math.DoubleV2?] = [
            grid[base.0 - 1], grid[base.0], grid[base.0 + 1],
            grid[
                base.1 - 2
            ], grid[base.1 - 1], grid[base.1], grid[base.1 + 1], grid[base.1 + 2],
            grid[
                center - 2
            ], grid[center - 1],               grid[center + 1], grid[center + 2],
            grid[
                base.2 - 2
            ], grid[base.2 - 1], grid[base.2], grid[base.2 + 1], grid[base.2 + 2],
            grid[base.3 - 1], grid[base.3], grid[base.3 + 1]
        ]
        for cell: Math.DoubleV2? in ring {
            guard let occupant: Math.DoubleV2 = cell else {
                continue
            }

            let dv: Math.DoubleV2 = Math.sub(occupant, candidate)

            guard Math.dot(dv, dv) > 1 else {
                return false
            }
        }
