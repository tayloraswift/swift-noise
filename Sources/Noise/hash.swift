...
public protocol TilingNoise: Noise {
...
extension HashedTilingNoise {
    public func amplitude_scaled(by factor: Double) -> Self {
        return Self(
            amplitude: self.amplitude * factor, frequency: self.frequency,
            permutation_table: self.permutation_table, wavelengths: self.wavelengths
        )
    }
    public func frequency_scaled(by factor: Double) -> Self {
        return Self(
            amplitude: self.amplitude, frequency: self.frequency * factor,
            permutation_table: self.permutation_table, wavelengths: self.wavelengths
        )
    }
    public func reseeded() -> Self {
    ...
    public func transposed(octaves: Int = 1) -> Self {
        return Self(
            amplitude: self.amplitude, frequency: self.frequency,
            permutation_table: self.permutation_table,
...
public struct RandomXorshift {
    private var state128: (UInt32, UInt32, UInt32, UInt32)
    ...
    public init(seed: Int) {
        self.state128 = (
            1,
            0,
            UInt32(truncatingIfNeeded: seed >> UInt32.bitWidth),
    ...
    /// generator state.
    public mutating func generate() -> UInt32 {
    ...
    public mutating func generate(less_than maximum: UInt32) -> UInt32 {
...
public struct PermutationTable {
    private let permut: [UInt8] // keep these small to minimize cache misses
    ...
    // UNDOCUMENTED
    public init(reseeding old_table: PermutationTable) {
        self.init(old_permut: old_table.permut, seed: 0)
    }

    private init(old_permut: [UInt8], seed: Int) {
    ...
    public func hash(_ h1: Int) -> UInt8 {
        return self.permut[h1 & 255]
    }
    /// Hash two integer values.
    public func hash(_ h1: Int, _ h2: Int) -> UInt8 {
        return self.permut[Int(self.hash(h1)) ^ (h2 & 255)]
    }
    /// Hash three integer values.
    public func hash(_ h1: Int, _ h2: Int, _ h3: Int) -> UInt8 {
